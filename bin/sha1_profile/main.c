...
static void s_profile_streaming_hash_at_chunk_size(
    ...
    size_t chunk_size,
    size_t alignment) {
    struct aws_hash *hash_impl = aws_sha1_new(allocator);
    AWS_FATAL_ASSERT(hash_impl);

    struct aws_byte_buf output_buf;
    AWS_FATAL_ASSERT(
        !aws_byte_buf_init(&output_buf, allocator, AWS_SHA1_LEN) && "allocation of output buffer failed!");
    ...
    fprintf(stdout, "SHA1 streaming computation took %" PRIu64 "ns\n", end - start);
...
static void s_profile_oneshot_hash(struct aws_allocator *allocator, struct aws_byte_cursor to_hash) {
    ...
    uint64_t start = 0;
    AWS_FATAL_ASSERT(!aws_high_res_clock_get_ticks(&start) && "clock get ticks failed.");
    AWS_FATAL_ASSERT(!aws_sha1_compute(allocator, &to_hash, &output_buf, 0) && "Hash computation failed");
    uint64_t end = 0;
    AWS_FATAL_ASSERT(!aws_high_res_clock_get_ticks(&end) && "clock get ticks failed");
    fprintf(stdout, "SHA1 oneshot computation took %" PRIu64 "ns\n", end - start);
    aws_byte_buf_clean_up(&output_buf);
}

static void s_run_profiles(struct aws_allocator *allocator, size_t to_hash_size, const char *profile_name) {
    fprintf(stdout, "********************* SHA1 Profile %s ************************************\n\n", profile_name);
...
int main(void) {
    ...
    struct aws_hash *hash_impl = aws_sha1_new(allocator);
    fprintf(stdout, "Starting profile run for Sha1 using implementation %s\n\n", hash_impl->vtable->provider);
