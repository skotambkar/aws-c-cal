...
static void s_profile_streaming_hash_at_chunk_size(
    ...
    if (alignment) {
        ...
        runningHash = aws_checksums_crc32c(unaligned_chunk.ptr, unaligned_chunk.len, runningHash);
    }
    ...
    while (to_hash_seeked.len) {
        ...
        runningHash = aws_checksums_crc32c(chunk_to_process.ptr, chunk_to_process.len, runningHash);
    }

    uint64_t end = 0;
    AWS_FATAL_ASSERT(!aws_high_res_clock_get_ticks(&end) && "clock get ticks failed");
    if (print) {
        fprintf(stdout, "CRC32C streaming computation took %" PRIu64 "ns\n", end - start);
    }
 }

static void s_profile_oneshot_hash(struct aws_byte_cursor to_hash) {
    uint64_t start = 0;
    uint32_t runningHash = 0;

    AWS_FATAL_ASSERT(!aws_high_res_clock_get_ticks(&start) && "clock get ticks failed.");
    runningHash = aws_checksums_crc32c(to_hash.ptr, to_hash.len, runningHash);

    uint64_t end = 0;
    AWS_FATAL_ASSERT(!aws_high_res_clock_get_ticks(&end) && "clock get ticks failed");
    fprintf(stdout, "CRC32C oneshot computation took %" PRIu64 "ns\n", end - start);
}

static void s_run_profiles(struct aws_allocator *allocator, size_t to_hash_size, const char *profile_name) {
    fprintf(stdout, "********************* CRC32C Profile %s ************************************\n\n", profile_name);
    ...
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 128, 8, false);
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 128, 8, true);
    fprintf(stdout, "16-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 128, 16, true);
    fprintf(stdout, "64-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 128, 64, true);
    fprintf(stdout, "128-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 128, 128, true);
    fprintf(stdout, "\n****** 256 byte chunks ******\n\n");
    fprintf(stdout, "8-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 256, 8, true);
    fprintf(stdout, "16-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 256, 16, true);
    fprintf(stdout, "64-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 256, 64, true);
    fprintf(stdout, "128-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 256, 128, true);

    fprintf(stdout, "\n******* 512 byte chunks *****\n\n");
    fprintf(stdout, "8-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 512, 8, true);
    fprintf(stdout, "16-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 512, 16, true);
    fprintf(stdout, "64-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 512, 64, true);
    fprintf(stdout, "128-byte alignment:\n");
    s_profile_streaming_hash_at_chunk_size(to_hash_cur, 512, 128, true);

    fprintf(stdout, "\n********************** Oneshot Run *******************************************\n\n");
    s_profile_oneshot_hash(to_hash_cur);
    fprintf(stdout, "\n\n");
...
int main(void) {
    ...
    fprintf(stdout, "Starting profile run for CRC32C using implementation \n\n");
